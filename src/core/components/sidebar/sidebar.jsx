...
const SidebarComp = ({ wide, isMatch, slide, slideOut }) => {
    ...
    return (
        <>
            ...
            <Box component='aside' className={`${sidebar.sidebar} ${wide && sidebar.newSidebar} ${isMatch && sidebar.slide_remove} ${slide && sidebar.slide_in}`}>

                {/* LOGO Session Start */}

                <Box>
                    {/* Side Header Session */}
                    <Stack direction='row' spacing={1} sx={{ mt: '10px', position: 'relative' }} justifyContent='center'>
                        ...
                        {isMatch && <FontAwesomeIcon icon={faXmark} onClick={slideOut} className={sidebar.closeNav} />}
